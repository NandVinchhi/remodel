      ...
      <Stack spacing="6">
        <Stack spacing="5">
          ...
          <FormControl isRequired>
            ...
            <Input value={confirm} onChange={e=> setConfirm(e.target.value)}id="confirmpassword" type="password" placeholder="********"/>
            { 
              password.length < 6 && (<FormHelperText color="fg.muted">At least 6 characters long</FormHelperText>)
            }
            { 
              password.length >= 6 && password != confirm && (<FormHelperText color="fg.muted">Passwords must match</FormHelperText>)
            }
        ...
        <Stack spacing="4">
          <Button onClick={async () => { 
              ...
              setLoading(false) 
            }} isDisabled={email.length == 0 || password.length < 6 || confirm.length < 0 || password != confirm} isLoading={loading}>Create account</Button>
